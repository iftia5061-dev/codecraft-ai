...
<body>
    ...
    <script>
        ...
        function appendMessage(role, text, isImage = false, save = true) {
            ...
            if (isImage) {
                div.innerHTML = `
                    <img src="${text}" alt="AI Image">
                    <div class="img-actions">
                        <div class="action-btn" onclick="downloadImg('${text}')">Download</div>
                        <div class="action-btn" onclick="shareImg('${text}')">Share</div>
                    </div>
                `;
            } else {
                let formattedText = text.replace(/\\n/g, '<br>')
                                        .replace(/\\*\\*(.*?)\\*\\*/g, '<b>$1</b>')
                                        .replace(/```(?:html|python|js)?([\s\S]*?)```/g, '<pre>$1</pre>');
